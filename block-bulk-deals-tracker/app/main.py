...
from . import companies, market, news, predeal
...
class Result:
    ...
    @property
    def notes(self) -> list:
        return self.market_notes + self.context_notes + self.news_notes
...
def _build(request: TrackerRequest) -> Result:
    ...
    # Three independent trips to the outside world: today's figures, the window
    # before each deal, and the news. Run together, the slowest one sets the wait.
...
def tracker_excel(request: TrackerRequest):
    result = _build(request)
    return _attachment(
        build_workbook(result.rows, result.metrics, result.news),
...
def tracker_pdf(request: TrackerRequest):
    ...
    return _attachment(
        build_pdf(result.rows, result.metrics, result.news, meta),
        "application/pdf",
