...
from . import companies, market, news
...
def _build(request: TrackerRequest) -> Result:
    ...
    metrics, notes = market.collect(targets)
    headlines, news_notes = news.collect(targets)
...
def _filename(chosen, start: date, end: date, minimum: float) -> str:
    if not chosen:
    ...
    return f"{scope}_{start:%Y%m%d}-{end:%Y%m%d}_min{int(minimum)}cr.xlsx"


@app.post("/api/tracker.xlsx")
def tracker_excel(request: TrackerRequest):
    result = _build(request)
    name = _filename(result.chosen, result.start, result.end, request.min_deal_size_cr)
    return Response(
        content=build_workbook(result.rows, result.metrics, result.news),
