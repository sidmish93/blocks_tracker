...
# Render sets RENDER=true. NSE's live quote API is almost always blocked from
# cloud IPs, so skipping it saves a failed round-trip per company.
ON_RENDER = bool(os.environ.get("RENDER"))
SKIP_NSE_LIVE = ON_RENDER or os.environ.get("SKIP_NSE_LIVE", "").lower() in (
    "1",
    "true",
    "yes",
)
...
# A bit higher on Render: the work is network-bound, and free-tier CPU is tiny.
METRIC_WORKERS = 10 if ON_RENDER else 6
...
# 90s hung every failed NSE call on Render; 25s is enough for a healthy reply.
