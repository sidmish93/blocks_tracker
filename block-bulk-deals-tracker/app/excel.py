...
MARKET_BASE_COLUMNS = [
    ("Company Name", "company_name", 26, None),
    ("CMP NSE (Rs)", "cmp", 13, MONEY),
    ("Intraday (%)", "intraday_return_pct", 13, PERCENT),
    ("Daily (%)", "daily_return_pct", 13, PERCENT),
    ("Volume NSE", "live_volume", 14, WHOLE),
    ("MCap (Rs cr)", "market_cap_cr", 14, MONEY),
    ("Return 1D (%)", "return_1d_pct", 14, PERCENT),
    ("Return 1W (%)", "return_1w_pct", 14, PERCENT),
    ("Return 1M (%)", "return_1m_pct", 14, PERCENT),
]
...
def _big_live_move(record) -> bool:
    ...
    if "highlight_row" in record:
        return bool(record.get("highlight_row"))
...
def _trigger_field(record, field: str) -> bool:
    flag = _TRIGGER_FIELDS.get(field)
    if not flag:
        return False
    if flag in record:
        return bool(record.get(flag))
    if field == "live_volume":
        return False
    value = record.get(field)
    if value is None:
        return False
    ...
    for record in records:
        ...
        for index, (_, field, _, number_format) in enumerate(columns, start=1):
            ...
            if big:
                ...
                elif _trigger_field(record, field):
                    cell.font = TRIGGER_BOLD
