...
_LIMITED_TAIL = re.compile(r"[\s,]*\b(?:Limited|Ltd\.?)\s*$", re.IGNORECASE)


def short_company_name(name: str) -> str:
    """Drop a trailing Limited / Ltd for compact market-table labels."""
    text = (name or "").strip()
    if not text:
        return ""
    while True:
        trimmed = _LIMITED_TAIL.sub("", text).rstrip(" ,")
        if trimmed == text:
            return text
        text = trimmed
