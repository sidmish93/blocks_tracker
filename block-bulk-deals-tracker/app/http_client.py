...
# Bot / auth blocks and missing resources do not recover with retries.
...
def nse_get_text(url: str, attempts: int = 3) -> str:
    ...
    # Keep the message short for the UI — the full URL is noise for readers.
...
def bse_get_json(url: str, attempts: int = 3):
    ...
    for attempt in range(attempts):
        ...
        if attempt + 1 < attempts:
            time.sleep(1 + attempt)
    raise RuntimeError(f"BSE request failed ({last_error})")
