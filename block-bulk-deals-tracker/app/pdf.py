...
# Twenty columns across the page. The widest value each one has to hold sets
# its width, so a ten-letter ticker never runs into the date beside it.
MARKET_WIDTHS = [47, 18, 14, 12, 12, 12, 12, 13] + [10] * 3 + [12] * 3 + [13] * 3 + [10] * 3
MARKET_FIXED = 8  # company … market cap, before the 1D/1W/1M families
MARKET_FAMILIES = [
    ("Return (%)", 8),
    ("ADTV (Rs cr)", 11),
    ("VWAP (Rs)", 14),
...
def _market_head() -> tuple:
    ...
    for label, start in MARKET_FAMILIES:
        top[start] = label
    top = [
        Paragraph(_escape(text), HEAD_CENTRE if index >= 3 else HEAD)
        for index, text in enumerate(top)
    ]
    second = [""] * MARKET_FIXED + [
        Paragraph(period, HEAD_CENTRE)
        for _ in MARKET_FAMILIES
        for period in ("1D", "1W", "1M")
    ]

    spans = [("SPAN", (column, 0), (column, 1)) for column in range(MARKET_FIXED)]
...
MARKET_NOTE = (
    "A current snapshot of each company: trailing figures and CMP / intraday / daily "
    "returns from NSE only (live quote when available, otherwise the same session as "
    "Close), plus market capitalisation from BSE. These figures describe the company "
    "at generate time and do not move with the period above."
)
DEALS_NOTE = (
...
QUARTER_NOTE = (
    "Latest quarterly results from NSE Integrated Filings. Takeaways show the absolute "
    "figure and the move versus a year earlier (YoY), or the previous quarter when that "
    "filing is missing (QoQ). The link opens the exchange results PDF where NSE has one."
)
...
def _story(rows, market_data, articles, meta, quarters=None) -> list:
    ...
    story += [Spacer(1, 16), _heading(2, "Latest quarter"), Spacer(1, 4)]
    story += _intro(QUARTER_NOTE, meta.get("quarter_notes") or [])
    story.extend(_quarter_blocks(quarters))

    story += [Spacer(1, 16), _heading(3, "Block & bulk deals"), Spacer(1, 4)]
    ...
    story += [Spacer(1, 16), _heading(4, "News"), Spacer(1, 4)]
    story += _intro(
