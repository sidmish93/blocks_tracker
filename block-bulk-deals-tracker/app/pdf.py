...
def _story(rows, market_data, articles, meta) -> list:
    story = _cover(meta)
    ...
    story += [PageBreak(), _heading(2, "Block & bulk deals"), Spacer(1, 4)]
    ...
    story += [PageBreak(), _heading(3, "News"), Spacer(1, 4)]
    story += _intro(
...
def build_pdf(rows, market_data=None, articles=None, meta=None) -> bytes:
    meta = meta or {}
    rows = rows or []
    ...
    _, total = _render(_story(rows, market_data, articles, meta), footer, 0)
    raw, _ = _render(_story(rows, market_data, articles, meta), footer, total)
    return raw
