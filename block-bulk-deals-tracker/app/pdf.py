...
# Content-sized base widths (mm). Fewer volume windows → larger uniform scale.
_COMPANY = 48
_LIVE = [14, 12, 12, 16]  # CMP, intraday, daily, mkt cap
_RETURNS = [11, 11, 11]
...
def _market_table(entries, windows=None) -> Table:
    ...
    body_size = 6.5 * scale
    pad = 2.5 * scale
    fixed = 5  # company, CMP, intraday, daily, mkt cap
    return_start = fixed
    ...
    for index, entry in enumerate(entries, start=2):
        row = [
            Paragraph(_escape(entry.get("company_name")), company),
