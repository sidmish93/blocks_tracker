...
archive, an NSE generate-time quote (CMP), and BSE market capitalisation.

Trailing 1D/1W/1M figures come from the latest NSE session. CMP / intraday /
daily returns are NSE-only (live quote when available, else the same history
session as Close) so they never mix in a BSE last price.
"""
...
def _quote_from_nse_live(symbol: str) -> dict | None:
    """NSE live quote-equity. Often blocked; callers use history instead."""
    ...
    if cached is None:
        payload = json.loads(nse_get_text(NSE_QUOTE_URL.format(symbol=urllib.parse.quote(symbol))))
...
def _quote_from_nse_history(sessions) -> dict | None:
    """Same NSE series as Close: last traded (or close), open, prior close."""
    if not sessions:
        return None
    latest = sessions[-1]
...
def fetch_live_quote(nse_symbol: str = "", sessions=None) -> dict:
    """CMP + intraday/daily returns — NSE only (live, else same-day history)."""
...
def _for_target(target: Target, start: date, end: date) -> dict:
    ...
    # CMP / Daily / Intraday stay on NSE only — never mix in a BSE last price.
    if target.nse_symbol:
        quote = fetch_live_quote(target.nse_symbol, sessions=sessions)
        result.update(quote)
...
def collect(targets) -> tuple:
    ...
    if quote_failed:
        warnings.append(f"NSE CMP unavailable right now for {_names(quote_failed)}.")

    for row in results:
        if row.get("quote_time"):
            source = row.get("quote_source") or "NSE"
            warnings.insert(0, f"NSE CMP from {source} ({row['quote_time']}).")
            break
