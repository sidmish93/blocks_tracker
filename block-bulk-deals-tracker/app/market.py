...
def _for_target(target: Target, start: date, end: date) -> dict:
    ...
    # Overlap NSE history with BSE live + mkt cap — big win on Render latency.
    cap_box: dict = {}
    quote_box: dict = {}
    helpers = []
    if target.bse_code:
        def _load_cap():
            try:
                cap_box["value"] = _market_cap_cr(target.bse_code)
            except Exception as exc:
                cap_box["error"] = exc

        def _load_bse_quote():
            try:
                quote_box["value"] = _quote_from_bse_live(target.bse_code)
            except Exception as exc:
                quote_box["error"] = exc

        helpers = [
            threading.Thread(target=_load_cap),
            threading.Thread(target=_load_bse_quote),
        ]
    ...
    if target.bse_code:
        if "error" in cap_box or cap_box.get("value") is None:
            result["_bse_failed"] = True
        result["market_cap_cr"] = cap_box.get("value")

    quote = None
    if target.nse_symbol and not SKIP_NSE_LIVE:
        try:
            quote = _quote_from_nse_live(target.nse_symbol)
        except Exception:
            quote = None
    bse_quote = quote_box.get("value")
    if not (quote and quote.get("cmp")) and bse_quote and bse_quote.get("cmp"):
        quote = bse_quote
