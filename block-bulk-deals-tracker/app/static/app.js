...
function renderMarketData(entries, notes) {
  ...
  const lines = [
    ...
    "Close, CMP, Intraday % and Daily % are NSE-only. CMP is the NSE last price (live when available, else that session’s last traded/close); Intraday % uses NSE open, Daily % uses the prior NSE close.",
  ...
  for (const entry of entries) {
    ...
    const signed = {
      5: entry.intraday_return_pct,
      6: entry.daily_return_pct,
      8: entry.return_1d_pct,
      9: entry.return_1w_pct,
      10: entry.return_1m_pct,
    };
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    renderQuarters(data.quarters, data.quarter_notes);
