...
def _fact(filing, *keys) -> float | None:
    """Read a numeric OneD value from an XBRL fact dictionary."""
    if filing is None:
        return None
    facts = getattr(filing, "raw_facts", None) or {}
    for key in keys:
        node = facts.get(key)
...
def _is_banking(filing) -> bool:
    """Banks file INTEGRATED_FILING_BANKING XBRL; Ind-AS revenue/EBITDA stay blank."""
    facts = getattr(filing, "raw_facts", None) or {}
    return "InterestEarned" in facts or "OperatingProfitBeforeProvisionAndContingencies" in facts


def _npa_pct(value: float | None) -> float | None:
    """XBRL stores NPA ratios as fractions (0.0128 → 1.28%)."""
    if value is None:
        return None
...
def _bank_takeaways(current, prior: FilingResult | None, versus: str) -> list:
    """Takeaways from banking XBRL (interest earned, NII, op. profit, PAT, NPA)."""
    interest = _fact(current, "InterestEarned")
    interest_exp = _fact(current, "InterestExpended")
    nii = interest - interest_exp if interest is not None and interest_exp is not None else None
    other = _fact(current, "OtherIncome")
    income = _fact(current, "Income")
    op_profit = _fact(current, "OperatingProfitBeforeProvisionAndContingencies")
    pat = _fact(
        current,
    ...
    pbt = _fact(current, "ProfitLossFromOrdinaryActivitiesBeforeTax")
    eps = _fact(
        current,
        "BasicEarningsPerShareAfterExtraordinaryItems",
        "BasicEarningsPerShareBeforeExtraordinaryItems",
    )
    gnpa = _npa_pct(_fact(current, "PercentageOfGrossNpa"))
    nnpa = _npa_pct(_fact(current, "PercentageOfNpa"))

    prior_interest = _fact(prior, "InterestEarned")
    prior_exp = _fact(prior, "InterestExpended")
    prior_nii = (
        prior_interest - prior_exp
        if prior_interest is not None and prior_exp is not None
        else None
    )
    prior_op = _fact(prior, "OperatingProfitBeforeProvisionAndContingencies")
    prior_pat = _fact(
        prior,
        "ProfitLossForThePeriod",
        "ProfitLossFromOrdinaryActivitiesAfterTax",
    ...
    prior_gnpa = _npa_pct(_fact(prior, "PercentageOfGrossNpa"))
    prior_nnpa = _npa_pct(_fact(prior, "PercentageOfNpa"))

    points = []

    if interest is not None:
        points.append(f"Interest earned {_money(interest)}{_move(_yoy(interest, prior_interest), versus)}")

    if nii is not None:
        points.append(f"Net interest income {_money(nii)}{_move(_yoy(nii, prior_nii), versus)}")

    if other is not None and (income is None or other / income >= 0.05):
        points.append(f"Other income {_money(other)}{_move(_yoy(other, _fact(prior, 'OtherIncome')), versus)}")

    if op_profit is not None:
        points.append(
            f"Operating profit (pre-provisions) {_money(op_profit)}"
            f"{_move(_yoy(op_profit, prior_op), versus)}"
        )

    if pat is not None:
        points.append(f"PAT {_money(pat)}{_move(_yoy(pat, prior_pat), versus)}")
    elif pbt is not None:
        points.append(
            f"PBT {_money(pbt)}{_move(_yoy(pbt, _fact(prior, 'ProfitLossFromOrdinaryActivitiesBeforeTax')), versus)}"
        )

    if eps is not None:
        points.append(f"EPS Rs {eps:,.2f}{_move(_yoy(eps, prior_eps), versus)}")

    if gnpa is not None or nnpa is not None:
        bits = []
        if gnpa is not None:
            bit = f"Gross NPA {gnpa:.2f}%"
            if prior_gnpa is not None and abs(gnpa - prior_gnpa) >= 0.05:
                direction = "down" if gnpa < prior_gnpa else "up"
                bit += f", {direction} {abs(gnpa - prior_gnpa):.2f} pt {versus}"
            bits.append(bit)
        if nnpa is not None:
            bit = f"Net NPA {nnpa:.2f}%"
            if prior_nnpa is not None and abs(nnpa - prior_nnpa) >= 0.05:
                direction = "down" if nnpa < prior_nnpa else "up"
                bit += f", {direction} {abs(nnpa - prior_nnpa):.2f} pt {versus}"
            bits.append(bit)
...
def takeaways(current: FilingResult, prior: FilingResult | None = None, versus: str = "YoY") -> list:
    ...
    Banks file a separate banking XBRL schema; those go through `_bank_takeaways`.
    """
    if _is_banking(current) and current.q_revenue is None and current.q_pat is None:
        return _bank_takeaways(current, prior, versus)
...
def _load_prior(symbol: str, filings, ended: date) -> tuple:
    ...
    if prior_row is None:
        ...
        if prior_row is not None:
            versus = "QoQ"
            # QoQ is marked on the row / in the takeaway wording — no per-name note.
...
def _for_target(target) -> tuple:
    """Returns (entry|None, status) where status is '', 'qoq', 'failed', or 'empty'."""
    ...
    if not symbol:
        # Unlisted trusts — blank in the table is enough.
        return None, "empty"
    ...
    try:
        ...
        _, prior, versus, _ = _load_prior(symbol, filings, ended)
        ...
        if not points:
            return entry, "empty"
        return entry, "qoq" if versus == "QoQ" else ""
    except Exception:
        return empty, "failed"
...
def collect(targets) -> tuple:
    ...
    for entry, status in found:
        if entry and (entry.get("takeaways") or entry.get("report_url") or entry.get("quarter")):
    ...
    if qoq:
        notes.append(
            "Where NSE has no year-ago filing, takeaways use the previous quarter "
            f"(marked QoQ on {qoq} {'name' if qoq == 1 else 'names'})."
        )
    if failed:
        notes.append(
            f"NSE filings unavailable right now for {failed} "
            f"{'name' if failed == 1 else 'names'} — try again shortly."
        )
