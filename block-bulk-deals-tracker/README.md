...
computed from NSE's daily price/volume/delivery archive plus a live Yahoo NSE
(`.NS`) quote for CMP, session moves, volume, and market cap. It appears above the deals
table and as a second **Market Data** sheet in the Excel file.

| Metric | How it is calculated |
| --- | --- |
| CMP NSE / Intraday % / Daily % | Live NSE quote via Yahoo; intraday vs open, daily vs previous close |
| Volume NSE / Market Cap NSE | Live NSE volume and market capitalisation via Yahoo (Rs crore) |
...
same as nothing having been delivered. Traded stats and live quote fields are blank for
BSE-only securities (mostly InvITs) and whenever the Yahoo NSE feed is unavailable. The
interface says which company and why.

## Latest quarter

Every company in the result also gets its latest quarterly filing from NSE Integrated
Filings: which quarter it is (Indian FY label plus calendar month), a link to the results
PDF on the exchange, and a few short takeaways.

Each takeaway pairs the absolute figure with how it moved: revenue, EBITDA (with margin
and margin shift), PAT, and EPS. Comparison is versus the same quarter a year earlier when
NSE has that filing. Some names do not — Adani Green’s Jun-2026 print has no Jun-2025 on
either NSE feed — and in that case the previous quarter is used instead, labelled **QoQ**,
with a note saying so. Extra lines only appear when they change the reading of the quarter
— profits growing faster or slower than sales, a material exceptional item, heavy other
income, a high finance-cost burden, FYTD totals, or a debt-equity ratio.

This is a current snapshot and does not move with the deal period you chose. It sits after
market data on the interface, and forms a **Quarterly Results** sheet in the Excel file.
...
publisher. It sits after the latest-quarter section, and forms a **News** sheet in the
Excel file where both the headline and the link cells are clickable.
...
order you would read them: market data, latest quarter, the deals, then the news. Each
section starts a new page and is bookmarked, so a reader can jump to one from the sidebar,
and every page is numbered and footed with the period and the companies it covers.
...
| NSE live CMP, volume, market cap | Yahoo Finance `quoteSummary` for `{symbol}.NS` |
...
endpoint is behind a bot filter and returns 403, so live CMP, volume, and market cap come
from Yahoo's NSE (`.NS`) quoteSummary feed instead.
