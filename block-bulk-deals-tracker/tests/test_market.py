...
from app.market import (  # noqa: E402
    ...
    _measure,
    _pct_move,
    _quote_from_yahoo_nse,
    _window,
    apply_adjustments,
    avg_volume_prior_7_calendar_days,
    fetch_live_quote,
    highlight_flags,
)
...
class LiveQuote(unittest.TestCase):
    ...
    def test_blank_when_yahoo_nse_quote_fails(self):
        original = market._quote_from_yahoo_nse
        market._quote_from_yahoo_nse = lambda symbol: None
        try:
            quote = fetch_live_quote("RELIANCE", bse_code="500325")
        finally:
            market._quote_from_yahoo_nse = original
        self.assertIsNone(quote["cmp"])
        self.assertIsNone(quote["intraday_return_pct"])
        self.assertIsNone(quote["daily_return_pct"])
        self.assertIsNone(quote["live_volume"])

    def test_yahoo_nse_quote_fills_cmp_returns_and_volume(self):
        payload = {
            "chart": {
                "result": [
                    {
                        "meta": {
                            ...
                            "chartPreviousClose": 1292.9,
                            "regularMarketVolume": 24813341,
                            "regularMarketTime": 1785923100,
                        },
                        "indicators": {"quote": [{"open": [1293.0], "volume": [24813341]}]},
                    }
                ]
            }
        }
        original_read, original_write, original_get = (
            market.read_cache,
            market.write_cache,
            market.plain_get_text,
        )
        market.read_cache = lambda *args, **kwargs: None
        market.write_cache = lambda *args, **kwargs: None
        market.plain_get_text = lambda url, attempts=2: json.dumps(payload)
        try:
            fields = _quote_from_yahoo_nse("RELIANCE")
        finally:
            market.read_cache, market.write_cache, market.plain_get_text = (
                original_read,
                original_write,
                original_get,
            )
        self.assertEqual(fields["cmp"], 1280.0)
        self.assertEqual(fields["open"], 1293.0)
        self.assertEqual(fields["previous_close"], 1292.9)
        self.assertEqual(fields["live_volume"], 24_813_341)
        self.assertEqual(fields["quote_source"], "NSE")
        self.assertAlmostEqual(fields["intraday_return_pct"], round((1280 / 1293 - 1) * 100, 2))
        self.assertAlmostEqual(fields["daily_return_pct"], round((1280 / 1292.9 - 1) * 100, 2))

    def test_bse_trading_is_market_cap_only(self):
        original_read, original_write, original_get = (
            market.read_cache,
            market.write_cache,
            market.bse_get_json,
        ...
        market.read_cache = lambda *args, **kwargs: None
        market.write_cache = lambda *args, **kwargs: None
        market.bse_get_json = lambda url: {"MktCapFull": "1800000.50", "TTQ": "10.41"}
        try:
            snap = market._bse_trading("500325")
        finally:
        ...
        self.assertEqual(snap["market_cap_cr"], 1800000.5)
        self.assertNotIn("live_volume", snap)

    def test_avg_volume_uses_sessions_in_prior_7_calendar_days(self):
        live = date(2026, 8, 6)
        # Window is (live − 7, live) = (30 Jul, 6 Aug) exclusive of both ends' equality
        # via day > cutoff and day < live_date; 30 Jul is the cutoff and is out.
        sessions = [
            session(date(2026, 7, 30), 100, 100),  # cutoff — excluded
            session(date(2026, 7, 31), 100, 200),
            session(date(2026, 8, 1), 100, 400),
            session(date(2026, 8, 5), 100, 600),
            session(date(2026, 8, 6), 100, 9999),  # live date — excluded
        ]
        average = avg_volume_prior_7_calendar_days(sessions, live)
        self.assertAlmostEqual(average, (200 + 400 + 600) / 3)

    def test_volume_spike_highlight_needs_live_above_1_5x_average(self):
        live = date(2026, 8, 6)
        sessions = [
            session(date(2026, 8, 1), 100, 1000),
            session(date(2026, 8, 4), 100, 1000),
            session(date(2026, 8, 5), 100, 1000),
        ]
        # avg 1000; 1.5x = 1500
        quiet = highlight_flags(
            {"live_volume": 1500, "volume_as_of": live.isoformat()}, sessions
        )
        self.assertFalse(quiet["highlight_volume"])
        spiked = highlight_flags(
            {"live_volume": 1501, "volume_as_of": live.isoformat()}, sessions
        )
        self.assertTrue(spiked["highlight_volume"])
        self.assertTrue(spiked["highlight_row"])
        self.assertNotIn("avg_volume", spiked)
