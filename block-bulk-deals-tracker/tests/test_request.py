...
class Filenames(unittest.TestCase):
    ...
    def named_file(self, names, suffix="xlsx"):
        chosen = self.named(names) if names else []
        return _filename(chosen, date(2026, 6, 1), date(2026, 6, 30), 200, suffix)

    def test_the_whole_market_is_unnamed(self):
        self.assertEqual(
            self.named_file(None), "block-bulk-deals_20260601-20260630_min200cr.xlsx"
        )

    def test_the_document_and_the_workbook_differ_only_in_their_suffix(self):
        self.assertEqual(
            self.named_file(None, "pdf"), "block-bulk-deals_20260601-20260630_min200cr.pdf"
        )

    def test_a_few_companies_are_listed_by_ticker(self):
        name = self.named_file("Delhivery; Biocon")
    ...
    def test_many_companies_are_counted_instead_of_listed(self):
        name = self.named_file("Delhivery; Biocon; MCX; Bajaj Auto")
        self.assertTrue(name.startswith("4-companies_block-bulk-deals_"))

    def test_punctuated_tickers_stay_filename_safe(self):
        self.assertTrue(self.named_file("Bajaj Auto").startswith("bajaj-auto_block-bulk-deals_"))


class Sections(unittest.TestCase):
    def test_at_least_one_section_is_required(self):
    ...
    def test_non_deal_runs_need_a_company(self):
        with self.assertRaises(Exception):
            TrackerRequest(
                from_date=date(2026, 6, 1),
                to_date=date(2026, 6, 30),
