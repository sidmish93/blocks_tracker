...
class TheDocument(unittest.TestCase):
    ...
    def test_the_three_sections_are_bookmarked_in_the_order_asked_for(self):
        self.assertEqual(
            bookmarks(self.build()), ["Market data", "Block & bulk deals", "News"]
        )

    def test_each_section_starts_its_own_page(self):
        self.assertEqual(pages(self.build()), 3)

    def test_a_search_that_found_nothing_still_explains_all_three_sections(self):
        raw = build_pdf([], [], [], {**META, "deal_count": 0, "total_value_cr": 0})
        self.assertEqual(pages(raw), 3)
        self.assertEqual(bookmarks(raw), ["Market data", "Block & bulk deals", "News"])
    ...
    def test_a_deal_with_no_month_behind_it_still_prints(self):
        blank = {**DEAL, "prev_close": None, "discount_pct": None, "pre_return_1m_pct": None,
                 ...
                 "buyers": ""}
        self.assertEqual(pages(self.build(rows=[blank])), 3)
