...
class Takeaways(unittest.TestCase):
    ...
    def test_banking_xbrl_uses_interest_nii_and_npa(self):
        def bank(**facts):
            return filing(raw_facts={key: {"OneD": value} for key, value in facts.items()})

        points = takeaways(
            bank(
                InterestEarned=355_419_600_000,
                InterestExpended=202_147_000_000,
                OtherIncome=76_708_600_000,
                Income=432_128_200_000,
                OperatingProfitBeforeProvisionAndContingencies=124_987_500_000,
                ProfitLossForThePeriod=76_565_100_000,
                BasicEarningsPerShareAfterExtraordinaryItems=24.55,
                PercentageOfGrossNpa=0.0128,
                PercentageOfNpa=0.0039,
            ),
            bank(
                InterestEarned=323_483_100_000,
                InterestExpended=182_386_900_000,
                OtherIncome=80_526_900_000,
                Income=404_010_000_000,
                OperatingProfitBeforeProvisionAndContingencies=122_138_900_000,
                ProfitLossForThePeriod=62_601_700_000,
                BasicEarningsPerShareAfterExtraordinaryItems=20.12,
                PercentageOfGrossNpa=0.0157,
                PercentageOfNpa=0.0045,
            ),
        )
        self.assertTrue(any(point.startswith("Interest earned Rs 35,541.96 cr") for point in points))
        self.assertTrue(any(point.startswith("Net interest income Rs 15,327.26 cr") for point in points))
        self.assertTrue(any("Operating profit (pre-provisions)" in point for point in points))
        self.assertTrue(any(point.startswith("PAT Rs 7,656.51 cr") for point in points))
        self.assertTrue(any("Gross NPA 1.28%" in point and "Net NPA 0.39%" in point for point in points))
